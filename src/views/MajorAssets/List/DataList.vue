<template>
    <el-table v-loading="isLoading" ref="tableRef" style="width: 100%" show-overflow-tooltip empty-text="请输入查询条件并点击查询"
        ...
        <el-table-column prop="assetClass" label="资产大类" fixed="left" :min-width="150" show-overflow-tooltip align="left"
...
import { getList ,querylist} from "@/api/majorAssets.js";
...
let getContentList = async (obj: any) => {
    ...
    let res;
    	if(obj.treeType == '3'){
    		 res = await querylist(obj);
    	}else{
    		 res = await getList(obj);
    	}
...
const spanMap = computed(() => {
    ...
    tableData.value.forEach((row, index) => {
        if (row.assetClass !== currentA) {
            currentA = row.assetClass;
            startIndex = index;
...
const arraySpanMethod = ({
    ...
    if (row.assetClass === row.assetSub) {
        if (columnIndex === 0) {
            return [1, 2]
...
const handleRowStyle = ({ row, rowIndex }: SpanMethodProps) => {
    ...
    if (isBold.includes(row.assetClass)) {
...
const rowClassName = ({ row, rowIndex }: SpanMethodProps) => {
    if (row.assetClass === "资产合计"|| row.assetClass === "管理规模合计" || row.assetClass === "净资产合计") {
        return 'customColor'
    } else {
        return ''
    }
