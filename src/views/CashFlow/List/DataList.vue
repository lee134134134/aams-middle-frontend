<template>
	<el-table v-loading="SearchLoading1" ref="tableRef" style="width: 100%" show-overflow-tooltip empty-text="暂无数据"
		:max-height="tableHeight" :data="tableData" :row-key="getRowKey" :header-cell-style="{
            background: '#E4EFFF',
            color: '#333333',
            height: '38px',
        }" border :tree-props="{ children: 'children' }" :expand-row-kes="expandedKeys"
		:row-class-name="setRowClassName" :row-style="handleRowStyle">
		<el-table-column prop="insuranceType" label="险种" min-width="180" show-overflow-tooltip align="left"
			fixed="left">
			<template #default="{ row }">
				{{ row.insuranceType }}
			</template>
		</el-table-column>
		<el-table-column v-for="(item, i) in tabcolums" :key="item.key" :prop="item.key" :label="item.name"
			:min-width="item.width" show-overflow-tooltip :align="item.align" :resizable="false" :fixed="item.fixed">
		</el-table-column>
		<template #empty>
			<div style="display:flex;justify-content: center; align-items:center;flex-direction: column;">
				<img src="../../../assets/images/empty.png" style="height:200px;" />
				<div>暂无数据</div>
			</div>
		</template>
	</el-table>
</template>

<script lang="ts" setup>
	import { ref, computed, nextTick } from "vue";
	import { data } from "./common.js";
	const tableRef = ref<TableInstance>();
	const tableData = ref([]);
	const tableHeight = ref(window.innerHeight - 280);
	const tabcolums = ref(data);
	const expandedKeys = ref([] as any)
	// 表格数据
	const props = defineProps({
		SearchLoading1: {
			type: Boolean,
			default: false,
		},
	});
	// 获取列表数据
	let getContentList = async (obj : any) => {
		tableData.value = obj || [];
		await nextTick();
		if (tableData.value.length > 0) {
			expandedKeys.value = tableData.value.filter(el => el.expand).map(item => item.insuranceType)
		}
		nextTick(() => {
			tableData.value.forEach(el => {
				if (el.expand) {
					tableRef.value?.toggleRowExpansion(el, true)
				}
			})
		})
	};
	//多选
	let getRowKey = (row : any) => {
		return row.insuranceTypePath;
	};


	interface User {
		id : string;
		name : string;
		amount1 : string;
		amount2 : string;
		amount3 : number;
		amount4 : number;
	}
	interface SpanMethodProps {
		row : User;
		column : TableColumnCtx<User>;
		rowIndex : number;
		columnIndex : number;
	}

	// const handleRowStyle = ({ row, rowIndex } : SpanMethodProps) => {
	// 	if (row.level === 0 && row.accountName !== '合计') {
	// 		return { fontWeight: 'bold', color: '#2173f7' }
	// 	} else if (row.level === 1 || row.accountName == '合计') {
	// 		return { fontWeight: 'bold', color: '#666' }
	// 	} else if (row.level === 2) {
	// 		return { color: '#666' }
	// 	} else {
	// 		return {}
	// 	}
	// }
	// const setRowClassName = ({ row, rowIndex }) => {
	// 	if (row.level === 0) {
	// 		return 'changeClass'
	// 	}
	// 	return ''
	// }


	const handleRowStyle = ({ row, rowIndex } : SpanMethodProps) => {
		if (row.level === 0 && row.insuranceType === '合计') {
			return { fontWeight: 'bold', color: '#2173f7' }
		} else if (row.level === 0) {
			return { fontWeight: 'bold', color: '#666' }
		} else {
			return { color: '#666' }
		}
	}
	const setRowClassName = ({ row, rowIndex }) => {
		if (row.level === 0) {
			if (row.insuranceType !== '合计' && row.insuranceType !== '资产占比') {
				return 'changeClass11'
			}
		} else if (row.level === 1) {
			return 'changeClass22'
		}
		return ''
	}
	defineExpose({
		getContentList
	});
</script>

<style scoped lang="scss">
	.btnArr {
		margin-bottom: 10px;
	}

	.upload-demo {
		width: 100%;
		height: 100%;
		display: flex;
		justify-content: center;
		align-items: center;
	}

	:deep(.el-switch--small .el-switch__core) {
		width: 45px !important;
	}

	:deep(.el-table__header-wrapper .el-table__header .el-table__cell) {
		border-right: 1px solid #ddd !important;
	}

	:deep(.el-table__header-wrapper .el-table__header .is-center) {
		border-bottom: 1px solid #ddd !important;
	}
</style>
<style>
	.el-table tr.changeClass11 {
		background-color: #f2f7ff !important;
	}

	.el-table tr.changeClass22 {
		background-color: #fff !important;
	}
</style>