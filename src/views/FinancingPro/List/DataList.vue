<template>
	<el-table v-loading="isLoading" ref="tableRef" style="width: 100%" show-overflow-tooltip empty-text="请输入查询条件并点击查询"
		...
		<el-table-column prop="accountName" label="受托子账户" min-width="230" show-overflow-tooltip align="left"
			fixed="left">
...
<script lang="ts" setup>
	...
	import { getList, } from "@/api/FinancingPro.js";
	...
	let getContentList = async (obj : any) => {
		...
		if (res && res.code === '00000') {
			...
			if (tableData.value.length > 0) {
				expandedKeys.value = tableData.value.filter(el => el.expand).map(item => item.accountName)
			}
			nextTick(() => {
	...
	const handleRowStyle = ({ row, rowIndex } : SpanMethodProps) => {
		if (row.level === 0 && (row.accountName === '合计' || row.accountName === '总计')) {
			return { fontWeight: 'bold', color: '#2173f7' }
		} else if (row.level === 0 || (row.level === 1&&row.children.length&&row.children.length>0)) {
	...
	const setRowClassName = ({ row, rowIndex }) => {
		if (row.level === 0) {
			if (row.accountName !== '合计'&&row.accountName !== '资产占比') {
				return 'changeClass11'
			}
		} else if (row.level === 1&&row.children.length&&row.children.length>0) {
