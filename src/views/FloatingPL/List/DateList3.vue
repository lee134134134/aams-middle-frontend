<template>
  ...
  <el-pagination v-model:current-page="currnetPage" v-model:page-size="pageSize" :page-sizes="[10, 20, 30, 50, 100]"
    ...
    data-track-id="FloatingPL-pagination3-btn" data-track-name="浮盈亏情况跟踪表-高股息-分页" @size-change="handleSizeChange"
...
watch(() => props.highDividendList, (newVal, oldVal) => {
  currnetPage.value = 1
  pageSize.value = 10
  pageAll.total = props.highDividendList.length;
...
const sortData = computed(() => {
  ...
  // })
  return tableData.value.map(item => ({ ...item })).sort((a, b) => {
    const { prop, order } = sortParams.value
    let valA = a[prop];
    let valB = b[prop];
    valA = parseNumber(valA)
