<template>
  ...
  <el-pagination v-model:current-page="currnetPage" v-model:page-size="pageSize" :page-sizes="[10, 20, 30, 50, 100]"
    ...
    data-track-id="FloatingPL-pagination4-btn" data-track-name="浮盈亏情况跟踪表-持仓个股-分页" @size-change="handleSizeChange"
...
watch(() => props.holdingList, (newVal, oldVal) => {
  currnetPage.value = 1
  pageSize.value = 10
  pageAll.total = props.holdingList.length;
...
const headerRowClassName = ({ row, column, rowIndex, columnIndex }) => {
  if (column.property === 'floatingProfitLossRate') {
    return {
      background: '#E4EFFF',
      color: '#2272F7',
      height: '38px'
