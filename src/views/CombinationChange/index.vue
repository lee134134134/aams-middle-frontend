					...
					<div class="subBtn">
						<el-form-item class=" margin-12">
							<el-button type="primary" @click="onSubmit(ruleFormRef)" icon="Search"
								...
								data-track-id="CombinationChange-submit-btn" data-track-name="组合结构变动表-查询">查询</el-button>
							...
							<el-button @click="handleDownload(ruleFormRef)" icon="Download" :loading="DownloadLoading"
								class="button-custom" data-track-id="CombinationChange-Download-btn"
								data-track-name="组合结构变动表-导出" :disabled="downloadBool">导出</el-button>
						</el-form-item>
					</div>
				</el-form>
			</div>
...
<script lang="ts" setup>
	...
	import { handleExportFile, getVariableStartAndEndDate } from "@/api/CombinationChange.js";
	...
	import { filterFormArr } from "@/utils/index.js";
	import { getChildrenCode, getValueArr1, filterTreeArr, compareObject, getTreeLabel } from "@/utils/index.js";
	...
	const getDefaultsDate = async () => {
		const res = await getVariableStartAndEndDate({ reqDateList: ['7', '2'] });
		if (res && res.code == '00000') {
			formInline.startDate = res.data[0] || '';
			formInline.endDate = res.data[1] || '';
			resetList.startDate = res.data[0] || '';
			resetList.endDate = res.data[1] || '';
		}
	};
	// 开始结束时间限制
